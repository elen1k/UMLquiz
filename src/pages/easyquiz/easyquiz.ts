...
@IonicPage()
@Component({
  selector: 'page-easyquiz',
  templateUrl: 'easyquiz.html',
})
export class EasyquizPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad EasyquizPage');

    this.user = this.userProvider.getUser(firebase.auth().currentUser);

    this.progress='easy';
    this.progresscount=this.user.easy;

    this.dataService.loadTrueFalse(1).then((data) => {
